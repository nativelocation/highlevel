<template>
	<nav class="hl_navbar shrink" id="navbar">
		<div class="hl_navbar--inner">
			<router-link to="/" class="hl_navbar--logo">
				<svg xmlns="http://www.w3.org/2000/svg" width="28" height="26">
					<image width="28" height="26" xlink:href="data:img/png;base64," />
				</svg>
			</router-link>
			<button class="hl_navbar--toggler" :class="navbarToggler ? 'active' : ''" type="button" id="navbar-toggler" @click="clickNavbarToggler">
				<span class="navbar-toggler-bar"></span>
				<span class="navbar-toggler-bar"></span>
				<span class="navbar-toggler-bar"></span>
			</button>
			<div class="hl_navbar--collapse" id="navbar-collapse" :style="navbarToggler ? 'display: block' : ''">
				<button class="hl_navbar--button btn btn-success btn-block" data-toggle="modal" data-target="#client-checkin--modal">Check In Client</button>
				<ul class="hl_navbar--links list-unstyled" id="nav-links">
					<li id="nav-dashboard">
						<router-link to="/" data-tooltip="nav-tooltip" data-placement="right" title="Dashboard">
							<i class="icon-duplicate"></i>
							<span>Dashboard</span>
						</router-link>
					</li>
					<li id="nav-widgets">
						<router-link to="/widgets" data-tooltip="nav-tooltip" data-placement="right" title="Widgets">
							<i class="icon-tiles-filled"></i>
							<span>Widgets</span>
						</router-link>
					</li>
					<li id="nav-marketing">
						<a data-toggle="collapse" href="#nav-marketing-collapse" role="button" aria-expanded="false" aria-controls="nav-marketing-collapse" data-tooltip="nav-tooltip" data-placement="right" title="Marketing">
							<i class="icon-mail"></i>
							<span>Marketing</span>
							<i class="caret icon-arrow-down-1"></i>
						</a>
						<div class="collapse nav-dropdown-links" id="nav-marketing-collapse">
							<ul>
								<li class="active">
									<router-link to="/customer-acquisition">Customer Acquisition</router-link>
								</li>
								<li>
									<router-link to="/triggers">Triggers</router-link>
								</li>
								<li>
									<router-link to="/workflow">Workflow</router-link>
								</li>
								<li>
									<router-link to="/form-builder">Form Builder</router-link>
								</li>
								<li>
									<router-link to="#">Product Adoption</router-link>
								</li>
								<li>
									<router-link to="#">Product Upsell</router-link>
								</li>
								<li>
									<router-link to="#">Content Library</router-link>
								</li>
								<li>
									<router-link to="#">Snapshot Widget</router-link>
								</li>
							</ul>
						</div>
					</li>
					<li id="nav-reviews">
						<router-link to="/reviews" data-tooltip="nav-tooltip" data-placement="right" title="Reviews">
							<i class="icon-bubble-filled"></i>
							<span>Reviews</span>
						</router-link>
					</li>
					<li id="nav-customers">
						<a data-toggle="collapse" href="#nav-customer-collapse" role="button" aria-expanded="false" aria-controls="nav-customer-collapse" data-tooltip="nav-tooltip" data-placement="right" title="Customers">
							<i class="icon-user"></i>
								<span>Customers</span>
							<i class="caret icon-arrow-down-1"></i>
						</a>
						<div class="collapse nav-dropdown-links" id="nav-customer-collapse">
							<ul>
								<li class="active">
									<router-link to="/contact-details">Contact Details</router-link>
								</li>
								<li>
									<router-link to="/customers">Customers</router-link>
								</li>
							</ul>
						</div>
					</li>
					<li id="nav-team">
						<router-link to="/team" data-tooltip="nav-tooltip" data-placement="right" title="Team">
							<i class="icon-heart"></i>
							<span>Team</span>
						</router-link>
					</li>
					<li id="nav-online-analysis">
						<router-link to="/online-analysis" data-tooltip="nav-tooltip" data-placement="right" title="Online Analysis">
							<i class="icon-pulse"></i>
							<span>Online Analysis</span>
						</router-link>
					</li>
					<li id="nav-opportunities">
						<router-link to="/opportunities" data-tooltip="nav-tooltip" data-placement="right" title="Opportunities">
							<i class="icon-blocks"></i>
							<span>Opportunities</span>
						</router-link>
					</li>
					<li id="nav-conversations">
						<router-link to="/conversations" data-tooltip="nav-tooltip" data-placement="right" title="Conversations">
							<i class="icon-bubble"></i>
							<span>Conversations</span>
						</router-link>
					</li>
					<li id="nav-calendar">
						<router-link to="/calendar" data-tooltip="nav-tooltip" data-placement="right" title="Calendar">
							<i class="icon-calendar"></i>
							<span>Calendar</span>
						</router-link>
					</li>
					<li id="nav-settings">
						<router-link to="/settings-profile" data-tooltip="nav-tooltip" data-placement="right" title="Settings">
							<i class="icon-settings-1"></i>
							<span>Settings</span>
						</router-link>
					</li>
					<li id="nav-social">
						<router-link to="/social" data-tooltip="nav-tooltip" data-placement="right" title="Social Media">
							<i class="icon-target-2"></i>
							<span>Social Media</span>
						</router-link>
					</li>
				</ul>
			</div>
		</div>
	</nav>
</template>

<script>
export default {
	name: 'NavBar',
	data: function () {
		return {
			navbarToggler: false,
		};
	},
	methods: {
		clickNavbarToggler (event) {
			this.navbarToggler = !this.navbarToggler
		}
	},
}
</script>
